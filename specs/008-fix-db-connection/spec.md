...
- The deployed Netlify Functions runtime already receives a working `NETLIFY_DATABASE_URL` (or equivalent) via the Netlify platform's database integration. The bug is specific to the local dev configuration introduced by the 007 split-dev-ports work, which moved away from `netlify dev` (which auto-injects linked-site env vars) to a different start command.
- Contributors have, or are willing to obtain, access to a Postgres database for local development — either via a Netlify-hosted dev branch, a local Postgres install, or another teammate-shared mechanism. Provisioning a database for new contributors is out of scope; this spec only requires that whatever database they have can be wired into the functions process.
- The existing dev port layout (SPA on 8888, functions on 9999) and the existing single-command start workflow from spec 007 remain authoritative. This fix MUST live inside that layout, not require returning to `netlify dev` as the front door.
- The `.env` mechanism (or equivalent local-only configuration) is acceptable for storing the dev connection string, provided it is gitignored and the file is not packaged into any build artifact.
